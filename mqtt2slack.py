...
def on_message(client, userdata, msg):
    ...
    logging.info(f"on_message : topic={msg.topic}, payload={payload_str}")
    res = requests.post(config["slack_api_url"],
            data={
                "token":   config["slack_api_token"], 
                "channel": "#"+config["slack_api_channel"],
                "text": payload_str
                })
    logging.info("send message to slack. channel="+config["slack_api_channel"]+", text="+payload_str)
